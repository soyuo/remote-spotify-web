...
function PreviousIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" focusable="false">
      <path d="M7 6v12" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="2" />
...
function NextIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 24 24" focusable="false">
      ...
      <path d="M17 6v12" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="2" />
    </svg>
  );
}
    ...
    <footer className={`player-bar ${isPlaying ? "is-playing" : ""}`} aria-label="재생 바">
      ...
      <div className="player-controls" aria-label="재생 컨트롤">
        <div className="player-buttons">
          <IconButton label="이전 곡" className="player-control-button" onClick={onPrevious}>
            <PreviousIcon />
          </IconButton>
          <IconButton label={isPlaying ? "일시정지" : "재생"} className="player-play-button" onClick={onToggle}>
            {isPlaying ? <PauseIcon /> : <PlayIcon />}
          </IconButton>
          <IconButton label="다음 곡" className="player-control-button" onClick={onNext}>
            <NextIcon />
          </IconButton>
        </div>
